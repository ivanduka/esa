...
def extract_csv_single(args):
    buf = StringIO()
    with redirect_stdout(buf), redirect_stderr(buf):
        pdf_id, page, engine_string, pdf_files_folder_string, csv_tables_folder_string = args
        single_pdf_files_folder = Path(pdf_files_folder_string)
        csv_tables_folder = Path(csv_tables_folder_string)
        engine = create_engine(engine_string)
        start_time = time.time()
        ...
        try:
            pdf_file_path = single_pdf_files_folder.joinpath(f"{pdf_id}_{page}.pdf")

            tables = camelot.read_pdf(str(pdf_file_path), pages='1', strip_text='\n',
                                      line_scale=40, flag_size=True, copy_text=['v'],)
            save_tables(tables, page, "lattice-v")

            duration = round(time.time() - start_time)
            print(f"{pdf_id}: done page {page} in {duration} seconds ({round(duration/60, 2)} min or {round(duration/3600, 2)} hours)")
        except Exception as e:
            print(f'Error processing {pdf_id} on page {page}:')
            print(e)
            traceback.print_tb(e.__traceback__)
